...
(function() {
    ...
    <div id="bj-container" style="width: 100%; max-width: 790px; height: 90px; margin: 10px auto; background: radial-gradient(circle at center, #1b2735 0%, #090a0f 100%); border-radius: 12px; box-shadow: 0 0 25px rgba(0,210,255,0.6); font-family: 'Segoe UI', Arial, sans-serif; overflow: hidden; border: 2px solid #00d2ff; position: relative; display: flex; align-items: center; padding: 0 10px; box-sizing: border-box; color: #fff;">
        ...
        <iframe id="bj-yt-iframe" src="" style="position: absolute; width: 1px; height: 1px; opacity: 0; pointer-events: none;" allow="autoplay"></iframe>
    ...
    var ytInfo = document.querySelector('#yt-audio-info span');
    ...
    async function toggle(type) {
        if (currentMode === type) { reset(); return; }
        reset();
        currentMode = type;
        ...
        if (type === 'yt') {
            document.getElementById('bj-status').innerText = "YT NEWS";
            if(!lastYtId) {
            ...
            ytIframe.src = `https://www.youtube.com/embed/${lastYtId}?autoplay=1&mute=0&enablejsapi=1`;
            ytBtnIcon.innerText = "⏸";
        } else {
    ...
    function reset() {
        audio.pause(); audio.src = ""; ytIframe.src = "";
        currentMode = '';
    ...
    document.getElementById('bj-yt-btn').onclick = () => toggle('yt');
    document.getElementById('bj-fm-btn').onclick = () => toggle('fm');
    document.getElementById('bj-vol').oninput = function() { audio.volume = this.value; };
